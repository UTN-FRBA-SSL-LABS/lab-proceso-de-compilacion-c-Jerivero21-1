...
> **R:El número es mucho más grande que las 94 líneas de programa.c porque programa.i es el archivo después del preprocesamiento. Ahí el compilador ya metió todo lo que venía en los #include, o sea, el contenido de las librerías, además de reemplazar los #define y sacar los comentarios. Entonces el archivo termina teniendo mucho más código del que escribiste originalmente, por eso la cantidad de líneas crece tanto.**
...
> **R:No encuentra nada porque en la etapa de preprocesamiento se eliminan todos los comentarios del código. Como esa frase estaba dentro de un comentario en programa.c, directamente no pasa al archivo programa.i, por eso grep no devuelve ningún resultado.**
...
> **R:# Las lineas N "archivo" son directivas que agrega el preprocesador para indicar de qué archivo original proviene el código que sigue y en qué línea estaba. Sirven para que el compilador sepan referenciar correctamente el origen del código después de haber expandido los #include.**
...
> **R:Esas instrucciones son el comienzo de la función. Preparan la pila para poder ejecutarla: guardan información del estado anterior y dejan todo listo para trabajar dentro de sumar sin romper lo que había antes.**
...
> **R:Porque en programa.o la función area_circulo no está implementada, solo se usa, entonces aparece como U (undefined). En cambio, en matematica.o sí está definida la función, por eso aparece como T, es decir, código de la función.**
...
```bash
nm programa nm| grep -E "sumar|factorial|area|main|llamadas|imprimir"
```
...
> **R: Se quedan porque son funciones de librerías del sistema. No se resuelven al compilar, sino cuando ejecutás el programa, ahí el sistema las completa automáticamente.**
...
> **R: Una macro como CUADRADO(x) no es una función real: es solo un reemplazo de texto que hace el preprocesador antes de compilar. En cambio, sumar(a, b) sí es una función de verdad, con su código, parámetros y llamada en tiempo de ejecución. La macro “desaparece” en el preprocesamiento porque se expande ahí mismo, mientras que la función real existe hasta el programa final. Además, la macro no tiene verificación de tipos, pero la función sí tiene chequeo de tipos en la compilación.**
...
> **R: Un símbolo T es código de una función, o sea, algo que está en la sección .text. Un símbolo D es una variable global inicializada, que vive en la sección .data. Básicamente, T = código ejecutable, D = datos ya inicializados.**
